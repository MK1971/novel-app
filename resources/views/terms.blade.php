            ...
            <div class="bg-white/80 backdrop-blur-sm overflow-hidden shadow-xl sm:rounded-2xl border border-amber-100">
                <div class="p-8 md:p-12">
                    ...
                    <div class="prose prose-amber max-w-none text-amber-800">
                        ...
                        <h2 class="text-2xl font-semibold text-amber-900 mt-10 mb-4">Who we are</h2>
                        <p class="mb-6">
                            Operator: <strong>{{ config('legal.entity_name') }}</strong>@if(filled(config('legal.entity_address'))) · {{ config('legal.entity_address') }}@endif. Contact channel: <a href="{{ route('feedback.index') }}" class="text-amber-700 underline hover:text-amber-900">Feedback</a>@if(filled(config('legal.contact_email'))) or <a href="mailto:{{ config('legal.contact_email') }}" class="text-amber-700 underline hover:text-amber-900">{{ config('legal.contact_email') }}</a>@endif.
                        </p>
                        ...
                        <h2 class="text-2xl font-semibold text-amber-900 mt-10 mb-4">Minimum age</h2>
                        <p class="mb-6">
                            You must be at least 13 years old to use the Service (or older where local law requires). If you are under the age of digital consent in your region, use the Service only with verifiable parent or guardian consent.
                        </p>
                        ...
                        <h2 class="text-2xl font-semibold text-amber-900 mt-10 mb-4">Governing law and disputes</h2>
                        <p class="mb-6">
                            These terms are governed by the laws of {{ config('legal.jurisdiction') }}. Before formal legal action, both sides agree to try informal resolution in good faith for at least {{ config('legal.dispute_notice_days') }} days after written notice.
                        </p>

                        <h2 class="text-2xl font-semibold text-amber-900 mt-10 mb-4">Changes to these terms</h2>
                        <p class="mb-6">
                            We may update these terms from time to time. For material changes, we will provide notice in-app or by email when practical before the changes take effect. The “Last updated” date reflects the current version.
                        </p>
