    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-amber-800/60 font-bold mt-1">Review and approve full chapter edit suggestions.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="grid gap-8">
                    ...
                    <div class="bg-white border border-amber-100 shadow-sm rounded-[3rem] p-10 hover:shadow-lg transition-all">
                        <div class="flex flex-wrap justify-between items-start gap-4 mb-8">
                            <div>
                                ...
                                <p class="text-amber-800/60 font-bold mt-1">By {{ $edit->user->name }} • {{ $edit->type }} • {{ $edit->created_at->diffForHumans() }}</p>
                            </div>
                        ...
                        <div class="grid md:grid-cols-2 gap-8 mb-10">
                            <div class="bg-amber-50/50 rounded-[2rem] p-8 border border-amber-100">
                                ...
                                <div class="text-amber-900/80 leading-relaxed whitespace-pre-wrap text-sm">{{ $edit->original_text }}</div>
                            </div>
                            <div class="bg-green-50/30 rounded-[2rem] p-8 border border-green-100">
                                ...
                                <div class="text-green-900 leading-relaxed font-medium whitespace-pre-wrap text-sm">{{ $edit->edited_text }}</div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-4 pt-8 border-t border-amber-50">
                            <form action="{{ route('admin.edits.approve', $edit) }}" method="POST" class="inline">
                                ...
                                <button type="submit" class="px-8 py-4 bg-green-600 text-white font-extrabold rounded-2xl hover:bg-green-700 transition-all shadow-xl shadow-green-600/20 transform hover:-translate-y-0.5">
                                    Accept Full (2 pts)
                                </button>
                            </form>
                            <form action="{{ route('admin.edits.approve', $edit) }}" method="POST" class="inline">
                                ...
                                <button type="submit" class="px-8 py-4 bg-blue-600 text-white font-extrabold rounded-2xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 transform hover:-translate-y-0.5">
                                    Accept Partial (1 pt)
                                </button>
                            </form>
                            <form action="{{ route('admin.edits.reject', $edit) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="px-8 py-4 bg-red-600 text-white font-extrabold rounded-2xl hover:bg-red-700 transition-all shadow-xl shadow-red-600/20 transform hover:-translate-y-0.5">
                                    Reject
                                </button>
                            </form>
                        </div>
                ...
                @empty
                    <div class="text-center py-32 bg-white border border-amber-100 rounded-[3rem] shadow-sm">
                        ...
                        <h3 class="text-2xl font-extrabold text-amber-900 mb-2">No pending edits to review</h3>
                        <p class="text-amber-800/50 text-lg font-bold">All suggestions have been processed. Good job!</p>
                    </div>
                @endforelse
            </div>
        </div>
